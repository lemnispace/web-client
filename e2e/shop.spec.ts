...
test.describe('Shop/Collections Page', () => {
  test('should load shop page successfully', async ({ page }) => {
    ...
    await page.waitForSelector('h3:has-text("Test")', { timeout: 10000 });

    // Take screenshot for manual review
    await page.screenshot({ path: 'playwright-report/shop-page.png', fullPage: true });
  });
  ...
  test('should display products from local database', async ({ page }) => {
    ...
    const productImages = page.locator('img[alt*="Canvas"], img[alt*="Shirt"], img[alt*="Test"]');
    const productTitles = page.locator('text=/Test.*Shirt|Test.*Canvas|Canvas Print/i');
    ...
    // Take screenshot
    await page.screenshot({ path: 'playwright-report/shop-products.png', fullPage: true });
  });

  test('should fetch products from local shop-api (Server-Side)', async ({ page }) => {
    ...
    const productTitles = page.locator('h3').filter({ hasText: /Test/ });
    ...
    await expect(page.getByText('Test Canvas Print').first()).toBeVisible();
  ...
  test('should navigate to product details when clicking a product', async ({ page }) => {
    ...
    const productLinks = page.locator('a[href*="/shop/products/"], a[href*="/products/"]');
    ...
    if (count > 0) {
      ...
      console.log('No product links found - products may not be loading correctly');
      await page.screenshot({ path: 'playwright-report/shop-no-products.png', fullPage: true });
